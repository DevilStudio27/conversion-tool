...
import * as JsXml from "js2xmlparser"
...
const IndexPage = () => {
    ...
    const formatCallback = () => {
        if (isValidFn(jsonInput)) {
            let data = JsXml.parse("root", JSON.parse(jsonInput));
            setJsonResult(data);
        }
        else {
                                ...
                                <form id="converterForm" >
                                    <div className="pretifier-container">
                                        ...
                                        <div className="btn-container">
                                            <button type="button" title="Convert to XML" className="btn btn-primary mr-2" onClick={() => formatCallback()}>Convert</button>
                                            <button type="button" title="Copy" className="btn btn-primary mr-2" name="copy" aria-label="Copy Json to Clipboard" onClick={() => handleCopy()}><FontAwesomeIcon icon={["fas", "copy"]} /> {isCopied === "copied" ? " Copied" : " Copy"}</button>
                                        </div>
                                        {
                                            jsonResult === null || jsonResult === "" || jsonResult === undefined ? " " :
                                                <div className="result-container">
                                                    <h5 className="res-title">XML Formatted Data</h5>
